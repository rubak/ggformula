 ...
 * adding term of the form `attribute:value` to the formula [auto chooses set or map]
 * adding a term of the form `attribute::value` [map]
...
(e.g. `"red"` or `0.5`, as appropriate), a variable (e.g. `cyl`), 
...
a variable in `data`, otherwise setting will be used.  The other two forms 
can be used to make things explicit
when this auto-detection method doesn't yield the desired results
...
## Plots with more than 2 required attributes
