...
variable an `gf_point()` that require two, there are several `gf_` functions that
require more than two variables.  How these positions are communicated via
slots in the formula is described by the "shape" listed in the quick help for 
these functions.  
...
gf_linerange(lo + hi ~ date | city ~ ., data = Temps, 
             color = ~ mid) %>%
  gf_labs(
    title = "Temparatures in three cities",
    subtitle = "2013 data",
...
LabData <- Temps %>% group_by(city) %>%
  summarise(date = date[which.max(hi)],
            hi = max (hi), 
            description = paste("Highest Hi:", hi, "on", date))
...
that makes the chainable with `ggformula`.  Those familiar with `ggplot2`
can replace
...
In the remainder of this tutorial, we illustrate some of the most common uses
of `gf_refine()`.
...
map back from colors to values when interpretting the plot.)
...
In the example below, we choose a rainbow of colors (and reverse the order) to
...
```{r, scales, exercise = TRUE, fig.height = 4.5}
gf_linerange(lo + hi ~ date | city ~ ., data = Temps, 
             color = ~ mid) %>%
...
```{r, scales2, exercise = TRUE, fig.height = 4.5}
gf_linerange(lo + hi ~ date | city ~ ., data = Temps, 
             color = ~ mid) %>%
...
```{r, brewer, exercise = TRUE, fig.height = 4.5}
...
As the name suggests, `coord_flip()` reverses the roles of the x- and y-axis.  This is most useful for glyphs that `ggplot2` only provides in a "vertical" version,
...
In this section we will use a different data set.  `Galton` contains data
